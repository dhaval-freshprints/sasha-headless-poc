...
- Shipping options: list every client-facing tier the page shows, each with its cost and date. Individual Shipping is internal; leave it out. The tree may put a tier's date on the line above its name; match them up. If a tier shows a saving instead of a cost, say it's the cheapest and give its date. If a tier's minimum is above the client's quantity, say so rather than dropping it.
...
- A proof that's still with the art team stays as it is: no revision, no alternatives on it. You can still price other products in the quoter if they're over budget or under the minimum.
